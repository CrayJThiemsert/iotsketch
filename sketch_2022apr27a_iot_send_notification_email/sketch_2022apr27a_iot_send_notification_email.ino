...
#define DEFAULT_NOTIFY_HUMID_LOWER 10
#define DEFAULT_NOTIFY_HUMID_HIGHER 80
#define DEFAULT_NOTIFY_TEMP_LOWER 10
#define DEFAULT_NOTIFY_TEMP_HIGHER 50
...
float gNotifyHumidHigher = 0;
float gNotifyTempLower = 0;
float gNotifyTempHigher = 0;
String gNotifyEmail = "";
...
bool processSendNotificationEmail(String notifyType) {
  ...
  if(notifyType.equals("humidity")) {
    // humidity
    messageSubject = "Humidity Notification";

    if(gHumidity < gNotifyHumidLower) {
      messageBody = "Humidity " + String(gHumidity) + " is lower than " + String(gNotifyHumidLower) + ".<br>Regards,<br>IoT System";  
    } else {
      if(gHumidity > gNotifyHumidHigher) {
        messageBody = "Humidity " + String(gHumidity) + " is higher than " + String(gNotifyHumidLower) + ".<br>Regards,<br>IoT System";  
      }
    }
    ...
    if(notifyType.equals("temperature")) {
      // temperature
      messageSubject = "Temperature Notification";
  
      if(gTemperature < gNotifyTempLower) {
        messageBody = "Temperature " + String(gTemperature) + " is lower than " + String(gNotifyTempLower) + ".<br>Regards,<br>IoT System";  
      } else {
        if(gTemperature > gNotifyTempHigher) {
          messageBody = "Temperature " + String(gTemperature) + " is higher than " + String(gNotifyTempHigher) + ".<br>Regards,<br>IoT System";  
        }
      }
    }
  }
...
bool processNotification(void) {
  ...
  if(gHumidity < gNotifyHumidLower || gHumidity > gNotifyHumidHigher) {
    ...
    processSendNotificationEmail("humidity");
  } else {
  ...
  if(gTemperature < gNotifyTempLower || gTemperature > gNotifyTempHigher) {
    ...
    processSendNotificationEmail("temperature");
  } else {
...
bool writeLocalNotificationInfo(void) {
  ...
  if(!writeLocalNotificationConfig(318, 10, String(gNotifyHumidLower))) {
    Serial.println("writeLocalNotificationConfig is failured!!!");
  }
