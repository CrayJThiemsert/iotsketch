...
void loop() {
  ...
  // calculate ledElapsedTime
   long ledElapsedTime = millis() - lastTime;
   lastTime = lastTime + ledElapsedTime;
//   Serial.print("main loop: lastTime=");
//   Serial.println(lastTime);
  Serial.println();   

   blinkLEDOnce(ledElapsedTime); // blink once every 1 second
  ...
  long normalProcessElapsedTime = millis() - normalProcessLastTime;
  normalProcessLastTime = normalProcessLastTime + normalProcessElapsedTime;
  Serial.print("normal process: normalProcessElapsedTime=");
  Serial.println(normalProcessElapsedTime);
  delay(1000);
  processNormalTasks(normalProcessElapsedTime);

//  listenResetButton(ledElapsedTime);
}
...
bool listenResetButton(long elapsedMS) {
  ...
  if(gIsResetButtonPress == HIGH) {
    Serial.print("Reset button is pressing.. ******");
