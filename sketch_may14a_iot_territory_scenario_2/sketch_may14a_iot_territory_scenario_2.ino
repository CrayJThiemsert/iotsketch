...
// ======================================================================
// EEPROM ADDRESS
#define ADDR_START_SSID 0
#define ADDR_END_SSID 32

#define ADDR_START_PASSWORD 32
#define ADDR_END_PASSWORD 96

#define ADDR_START_FBHOST 96
#define ADDR_END_FBHOST 160

#define ADDR_START_FBAUTH 160
#define ADDR_END_FBAUTH 224

#define ADDR_START_MODE 224
#define ADDR_END_MODE 244

#define ADDR_START_DNAME 244
#define ADDR_END_DNAME 308

#define ADDR_START_INTERVAL 308
#define ADDR_END_INTERVAL 318

#define ADDR_START_USER_UID 318
#define ADDR_END_USER_UID 398

#define ADDR_START_SCENARIO 398
#define ADDR_END_SCENARIO 400

#define ADDR_START_MOBILESERVER 400
#define ADDR_END_MOBILESERVER 415
...
// ======================================================================
// Scenario
#define SCENARIO_ANYWHERE "1"
#define SCENARIO_WIFI_TERRITORY "2"
#define SCENARIO_DIRECTLY "3"
// ======================================================================
// Variables
int i = 0;
int statusCode;
...
String gUserUid = "";
String gScenario = SCENARIO_ANYWHERE;
String gMobileServer = "";
...
bool saveDataToMobileServer(void);
...
const char* mobilehost = "www.example.com"; //"192.168.1.106:8080/say-hi/shushi";
...
bool loadInternalConfig(void) {
  ...
  String esid = "";
  for (int i = ADDR_START_SSID; i < ADDR_END_SSID; ++i)  {
  ...
  String epass = "";
  for (int i = ADDR_END_SSID; i < ADDR_END_PASSWORD; ++i)  {
  ...
  String efbhost = "";
  for (int i = ADDR_END_PASSWORD; i < ADDR_END_FBHOST; ++i)  {
  ...
  String efbauth = "";
  for (int i = ADDR_END_FBHOST; i < ADDR_END_FBAUTH; ++i)  {
  ...
  String emode = "";
  for (int i = ADDR_END_FBAUTH; i < ADDR_END_MODE; ++i)  {
  ...
  String ename = "";
  for (int i = ADDR_END_MODE; i < ADDR_END_DNAME; ++i)  {
  ...
  String einterval = "";
  for (int i = ADDR_END_DNAME; i < ADDR_END_INTERVAL; ++i)  {
  ...
  String euseruid = "";
  for (int i = ADDR_END_INTERVAL; i < ADDR_END_USER_UID; ++i)  {
    euseruid += char(EEPROM.read(i));
  }
  Serial.print("USER UID: ");
  Serial.println(euseruid);
  gUserUid = euseruid;

  String escenario = "";
  for (int i = ADDR_END_USER_UID; i < ADDR_END_SCENARIO; ++i)  {
    escenario += char(EEPROM.read(i));
  }
  Serial.print("SCENARIO: ");
  Serial.println(escenario);
  gScenario = escenario;

  String emobileserver = "";
  for (int i = ADDR_END_SCENARIO; i < ADDR_END_MOBILESERVER; ++i)  {
    emobileserver += char(EEPROM.read(i));
  }
  Serial.print("MOBILE SERVER: ");
  Serial.println(emobileserver);
  gMobileServer = emobileserver;
  ...
  if(readSensor()) {
    ...
    switch(gScenario.toInt()) {
      ...
      case 2: {
          ...
          if(saveDataToMobileServer()) {
            Serial.println("Successfully Update data to the Mobile Server!!!");
            Serial.println("");
            result = true;
          } else {
            Serial.println("Update data to the Mobile Server Failured!!!");
          }
        }
        break;
      case 3: {
...
void createWebServer(void) {
    ...
    server.on("/setting", []() {
      ...
      String qscenario = server.arg("scenario");
      String qmobileserver = server.arg("mobileserver");
...
//        for (int i = 0; i < 400; ++i) { // + scenario 2 characters
//        for (int i = 0; i < 415; ++i) { // + scenario 15 characters
        for (int i = 0; i < ADDR_END_MOBILESERVER; ++i) { // + scenario 15 characters
          EEPROM.write(i, 0);
        }
        Serial.println(qsid);
        Serial.println("");
        ...
        for (int i = 0; i < qpass.length(); ++i) {
          EEPROM.write(ADDR_END_SSID + i, qpass[i]);
          Serial.print("Wrote: ");
      ...
      if (qfbhost.length() > 0 && qfbauth.length() > 0) {
        ...
        // Clear part
        for (int i = 0; i < 64; ++i) { 
          EEPROM.write(ADDR_END_PASSWORD + i, 0);
        }
        Serial.println("writing eeprom fbhost:");
        for (int i = 0; i < qfbhost.length(); ++i) {
          EEPROM.write(ADDR_END_PASSWORD + i, qfbhost[i]);
          Serial.print("Wrote: ");
        ...
        // Clear part
        for (int i = 0; i < 64; ++i) { 
          EEPROM.write(ADDR_END_FBHOST + i, 0);
        }
        Serial.println("writing eeprom fbauth:");
        for (int i = 0; i < qfbauth.length(); ++i) {
          EEPROM.write(ADDR_END_FBHOST + i, qfbauth[i]);
          Serial.print("Wrote: ");
      ...
      if (qmode.length() > 0) {
        ...
        // Clear part
        for (int i = 0; i < 20; ++i) { 
          EEPROM.write(ADDR_END_FBAUTH + i, 0);
        }
        Serial.println("writing eeprom mode:");
        for (int i = 0; i < qmode.length(); ++i) {
          EEPROM.write(ADDR_END_FBAUTH + i, qmode[i]);
          Serial.print("Wrote: ");
      ...
      if (qname.length() > 0) {
        ...
        // Clear part
        for (int i = 0; i < 64; ++i) { 
          EEPROM.write(ADDR_END_MODE + i, 0);
        }
        Serial.println("writing eeprom device name:");
        for (int i = 0; i < qname.length(); ++i) {
          EEPROM.write(ADDR_END_MODE + i, qname[i]);
          Serial.print("Wrote: ");
      ...
      if (quseruid.length() > 0) {
        ...
        // Clear part
        for (int i = 0; i < 80; ++i) { 
          EEPROM.write(ADDR_END_INTERVAL + i, 0);
        }
        Serial.println("writing eeprom user uid:");
        for (int i = 0; i < quseruid.length(); ++i) {
          EEPROM.write(ADDR_END_INTERVAL + i, quseruid[i]);
          Serial.print("Wrote: ");
          Serial.println(quseruid[i]);
        }
      }

      // save scenario into eeprom
      if (qscenario.length() > 0) {
        hasValues = true;
        // scenario size 2 characters.
        // Clear part
        for (int i = 0; i < 2; ++i) { 
          EEPROM.write(ADDR_END_USER_UID + i, 0);
        }
        Serial.println("writing eeprom scenario:");
        for (int i = 0; i < qscenario.length(); ++i) {
          EEPROM.write(ADDR_END_USER_UID + i, qscenario[i]);
          Serial.print("Wrote: ");
          Serial.println(qscenario[i]);
        }
      }

      // save mobile server into eeprom
      if (qmobileserver.length() > 0) {
        hasValues = true;
        // scenario size 15 characters.
        // Clear part
        for (int i = 0; i < 15; ++i) { 
          EEPROM.write(ADDR_END_SCENARIO + i, 0);
        }
        Serial.println("writing eeprom scenario:");
        for (int i = 0; i < qmobileserver.length(); ++i) {
          EEPROM.write(ADDR_END_SCENARIO + i, qmobileserver[i]);
          Serial.print("Wrote: ");
          Serial.println(qmobileserver[i]);
        }
      }
...
//    - uid => 2021-05-09 10:45:14
//    - humidity => 65.78
//    - temperature => 28.23
//    - readVoltage => 3.05

    String getData;
//    getData = "?uid=" + String("2021-05-09 10:45:14") + 
    getData = "?uid=" + String("2021-05-09%2010:45:14") + 
    "&humidity=" + "65.78"
    "&temperature=" + "28.23"
    "&readVoltage=" + "3.05";

    String htHistoryUrl = "http://" + gMobileServer + ":9090/wave"; // + getData;
    
    Serial.println("http://192.168.1.106:9090/user/2");
    
 
//    http.begin("http://192.168.1.106:9090/user/2");  //Specify request destination - OK
//    http.begin("http://192.168.1.106:9090/wave");  //Specify request destination - OK
//    http.begin("http://192.168.1.106:9090/wave?uid=2021-05-09%2010:45:14&humidity=65.78&temperature=28.23&readVoltage=3.05"); // - ok
//    htHistoryUrl = "http://192.168.1.106:9090/wave?uid=2021-05-09%2010:45:14&humidity=65.78&temperature=28.23&readVoltage=3.05"; // - ok
//    htHistoryUrl = "http://192.168.1.106:9090/wave?uid=2021-05-09 10:45:14&humidity=65.78&temperature=28.23&readVoltage=3.05"; // - ok
    Serial.println("htHistoryUrl=");
    Serial.println(htHistoryUrl);
//    http.begin(htHistoryUrl);  //Specify request destination
      http.begin(wifiClient, htHistoryUrl);  //Specify request destination
//    http.begin(htHistoryUrl.c_str());  //Specify request destination
...
////  Serial.printf("\n[Connecting to %s ... ", gMobileServer);
//  Serial.print("\n[Connecting to ... ");
//  Serial.println(gMobileServer);
////  if (client.connect(gMobileServer + "/wave", 8080))
//  if (client.connect(gMobileServer + "/wave", 9090))
//  {
...
//    String hostname = "http://" + gMobileServer + ":8080/say-hi/sushi";
//    http.begin(client, hostname); //HTTP
...
bool writeLocalReadingIntervalConfig(void) {
  ...
  for (int i = 0; i < gDeviceReadingInterval.length(); ++i) {
    EEPROM.write(ADDR_END_DNAME + i, gDeviceReadingInterval[i]);
    Serial.print("Wrote: ");
...
bool writeLocalUserUidConfig(void) {
  ...
  for (int i = 0; i < gUserUid.length(); ++i) {
    EEPROM.write(ADDR_END_INTERVAL + i, gUserUid[i]);
    Serial.print("Wrote: ");
